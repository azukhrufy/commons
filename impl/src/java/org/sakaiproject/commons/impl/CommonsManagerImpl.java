...
import org.sakaiproject.commons.api.datamodel.Like;
...
public class CommonsManagerImpl implements CommonsManager, Observer {
    ...
    //Tambahan Method yang dilanjutkan
    public Like saveLike(String commonsId, Like like){
        try{
            Post post = persistenceManager.getPost(like.getPostId(), false);
            if(commonsSecurityManager.canCurrentUserLikeOnPost(post)){
                Like savedLike = persistenceManager.saveLike(like);
                // Perkiraan ini berfungsi untuk menyimpan string ke dalam list
                // maksudnya list of comment.
                // untuk like sebenarnya hanya perlu variabel yang menampung
                // berapa like yang telah didapat saja, bukan menampung string
                // kedalam list.
                    
                // namun ketika var contextIds dijadikan integer maka, akan mempengaruhi 
                //conditional statement di bawahnya
                if(savedLike != null){
                    List<String> contextIds = new ArrayList();
                    if(persistenceManager.getCommons(commonsId).isSocial()){
                        contextIds = getConnectionUserIds(post.getCreatorId());
            ...
            log.error("Caught exception whilst saving like", e);
        }
        return null;
    }
